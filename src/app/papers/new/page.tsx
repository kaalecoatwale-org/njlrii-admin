...
export default function NewPaperPage() {
  ...
  useEffect(() => {
    ...
    const loadIssues = async () => {
      setDbLoading(true);
      try {
        const { data, error: issError } = await supabase
          .from('issues')
        ...
        setError(err.message || 'Failed to load releases metadata.');
      } finally {
        setDbLoading(false);
      }
    };
  ...
  if (loading || (user && !profile)) return <PageLoader message="Loading publish form..." />;
  ...
  // Auto Generate URL Friendly Slug from Title input
  ...
  // Add/Remove dynamic author index cards
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Filter out blank author entries
    ...
    try {
      ...
      setError(err.message || 'Failed to save paper record.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  return (
    <DashboardShell pageTitle="Publish Manuscript" currentRoute="/papers">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div className="admin-card">
          <div className="card-header-block">
            <h2 className="card-heading-title">📝 Publish Research Manuscript</h2>
          </div>
            ...
            <form onSubmit={handleSubmit} className="form-grid-layout">
              ...
              <div className="form-field-group col-span-12">
                ...
                <input
                  ...
                  placeholder="e.g. The Specific Relief (Amendment) Act: A Critical Appraisal"
              ...
              <div className="form-field-group col-span-6">
                ...
                <select
                  ...
                  required
                >
                  <option value="">{dbLoading ? 'Loading releases...' : 'Select Release Issue...'}</option>
                  {issues.map((iss) => (
              ...
              <div className="form-field-group col-span-12" style={{ display: 'flex', justifyContent: 'flex-end', flexDirection: 'row', gap: '12px', marginTop: '24px', borderTop: '1px solid var(--border)', paddingTop: '24px' }}>
                ...
                <button type="submit" className="btn-action primary" disabled={actionLoading || isUploading} style={{ cursor: 'pointer' }}>
                  {actionLoading ? 'Saving Publication...' : isUploading ? 'Uploading PDF...' : '💾 Save Publication Record'}
                </button>
              </div>
